...
| `data/remote/FirebaseSyncCommandExecutor.kt` | Firestore transport behind `SyncCommandExecutor` (contract: **commandId is the idempotency key** — satisfied by document identity + create-or-dedup-or-refuse transaction); schema-version gate, `Conflict` instead of blind overwrite |
| `data/sync/SyncWorker.kt` | `@HiltWorker` drain: pop-one FIFO, per-command `SyncResult` outcomes (Accepted→delete, Retryable→durable deferral + pause, Conflict→'CONFLICT' park, PermanentFailure→'DEAD' park), fixed attempt budget, WorkManager CONNECTED constraint |
