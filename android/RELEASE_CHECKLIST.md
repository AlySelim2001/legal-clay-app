...
2. **Evidence capture instrumentation test — ✅ مُنفّذ** (`app/src/androidTest/.../EvidenceCaptureInstrumentedTest.kt`):
   - جهاز/محاكي فقط (`Context` + `filesDir` حقيقيان)، لا يعمل على JVM — يشغَّل مثل اختبار الهجرة عبر `connectedDebugAndroidTest`.
   - يثبت المسار الحرج كاملًا عبر `captureAndSecureEvidence`: hash مدموج أثناء النسخ (متجه `"hello"` الموثّق في `Sha256Test`) → fsync → read-only → rename إلى `evidence/immutable/` → إدراج Room → حدث CAPTURED المؤسِّس، ثم فحص Red Team حقيقي (عبث بالملف المؤمَّن فعليًا ومقارنة hash) وفشل نظيف بلا ملفات متبقية (مصدر غير موجود + تكرار UNIQUE(originalFileHash))، ومرحلة OCR (حدث OCR_PROCESSED من رأس السلسلة + ختم `processedFileHash` + رفض المشتق الثاني).
   - يستخدم Room in-memory بمحرك SQLite العادي عمدًا (وليس SQLCipher): طبقة التشفير معزولة عن عقد سلسلة العهدة، ومكتبة sqlcipher JNI غير مُغلَّفة في APK الاختبار.
3. **SyncWorker integration test — ✅ مُنفّذ** (`app/src/test/.../data/sync/SyncWorkerTest.kt`) — عبر `androidx.work:work-testing` (`TestListenableWorkerBuilder` + `WorkManagerTestInitHelper`، يعمل على JVM مع Robolectric على SDK 35 — انظر الملاحظة أدناه) مع fake لـ `SyncCommandExecutor` وقائمة انتظار Room حقيقية في الذاكرة (الـSQL الإنتاجي هو ما يُختبر): يغطي `Accepted → حذف`، `Retryable → تأجيل durable` (الميزانية + `lastError` + `nextAttemptAt`، والصف غير مرئي للـdrain داخل نافذة التأجيل)، `Conflict → CONFLICT`، `PermanentFailure → DEAD`، وصف غير قابل للفك → DEAD **دون حجب ما خلفه** (اختبار livelock عمدًا بصف تالف في المقدمة)، وصفًا استنفد `MAX_ATTEMPTS=8` → DEAD **دون** لمس النقل، وقائمة فارغة → نجاح فوري.
   - ملاحظات تشغيل: Robolectric 4.16 يدعم SDK 36 لكنه يشترط JDK 21 لهدف 36 تحديدًا؛ سلم التحقق يعتمد JDK 17، لذا ثُبِّت الاختبار على SDK 35 (الـdrain المستقل عن نسخة النظام) — ارفعه إلى 36 مع JDK 21 في نفس التعديل. Robolectric يُنشئ SQL على أول تشغيل، وأُضيفت `testOptions.unitTests.isIncludeAndroidResources = true` والتبعيات `robolectric`/`work-testing`/`test-core`.
