...
- **الفحص الجنائي الاسترشادي (متاح اليوم ✅):** مقارنة SSIM عبر OpenCV تكشف مكوّنات الفروق الموضعية وفرق كثافة الحبر (مؤشرات الإضافات اللاحقة على التوقيع) — مُعايَرة على اختبارات اصطناعية: إضافة رقم واحد تُلتقط رغم بقاء التطابق الكلي ≈0.999. النتائج استرشادية فقط، والإثبات النهائي بخبرة مصلحة الطب الشرعي.
- **كشف الأختام والتوقيعات (YOLO — قيد التدريب 🚧):** نقطة النهاية جاهزة في الخدمة وتُفعَّل تلقائياً عند تركيب النموذج المدرَّب؛ حتى ذلك الحين تُعلن صراحةً أنها غير مفعّلة ولا تُنتج نتائج مختلقة — راجع [خارطة الطريق](#-خارطة-الطريق--roadmap).
- **ما هو متاح اليوم أيضاً:** استخراج نص موثوق من المحرر، تحليله عبر محرك RAG القانوني، وتمييز البنية الشرعية/الشكلية الناقصة في إيصال الأمانة (ركن التسليم، توصيف المحرر، البيانات الإلزامية).
...
[مستند/إيصال أمانة مشبوه أو محضر كيدي]
        │
        ▼
[PaddleOCR عربي + فحص SSIM] ──► استخراج النص + مؤشرات التلاعب الاسترشادية
        │
        ▼
...
```mermaid
flowchart LR
    DOC["مستند مشبوه<br/>(صورة/مسح ضوئي)"] --> OCR["paddle-ocr-service<br/>(OCR عربي + فحص SSIM)"]
...
### 🧠 منصة المعرفة القانونية (Evidence-First Web Platform)

منصة ويب كاملة مبنية داخل تطبيق CRIM-SYS (React + Convex) تعمل بمبدأ **«لا مصدر → لا ادعاء قانوني»**:

| المسار | الوصف |
|--------|-------|
| `/app/legal` | الصفحة الرئيسية: حالة المعرفة والأنشطة والأمانة العلمية |
| `/app/legal/ask` | **اسأل القانون** — إجابات مُركّبة حصراً من الأدلة المسترجعة (بدون LLM في مسار الإجابة)، مع إسناد كل ادعاء لمصدره وحالة دعمه ومؤشرات الثقة |
| `/app/legal/search` | بحث هجين (لفظي + دلالي) في المواد المنشورة مع وسم الصلاحية الزمنية |
| `/app/legal/next-steps` | **ماذا أفعل الآن؟** — أدلة إجرائية: خطوات، جهة مختصة، مستندات، تحذيرات |
| `/app/legal/authorities` | دليل الجهات الرسمية وخدماتها (لا تُخترع بيانات محلية أبداً) |
| `/app/legal/rights` | **اعرف حقك وواجباتك** — توعية قانونية مبنية على المصادر |
| `/app/legal/documents` | تحليل المستندات: استخراج التواريخ/المبالغ/المراجع + فحص حقن التعليمات (المستند = بيانات فقط) |
| `/app/legal/dossier` | ملف قضائي شخصي: خط زمني (أقوال/وقائع/مستندات)، ملاحظات، مهام |
| `/app/legal/sources` | سجل المصادر: النسخة، نطاق السريان، حالة التحقق، الرابط الرسمي |
| `/app/legal/history` | سجل الأسئلة السابقة مع حالة أدلة كل إجابة |
| `/admin/knowledge` | **مراجعة بشرية إلزامية**: طابور المراجعة → اعتماد → نشر (الذكاء الاصطناعي لا ينشر أبداً) + سجل تدقيق كامل |
| `/admin/evaluation` | تشغيل 10 اختبارات معيارية (TEST-001…010) على خط الأدلة الحقيقي: الامتناع، رفض الاختراع، كشف التعارض، الصلاحية الزمنية، وصول IDOR، تعامل المستند كبيانات |

**ضمانات الأمان:** تحقق ملكية صارم (T-012)، تحديد معدل الاستعلامات، إخفاء PII قبل التخزين، فحص حقن التعليمات في المستندات (T-002)، سجل تدقيق لكل إجراء (P-09)، وفصل الأدوار (مستخدم/مراجع/مدير) مع إدارة صلاحية مُدقَّقة.
...
> **حالة الإنجاز الفعلية: 8/10 محاور مكتملة ومُختبرة** — كل بند [x] مدعوم باختباراته في `local-ai/tests/`؛ والبندان المتبقيان موثّقان بدقة لأن المشروع لا يُعلن إنجازاً غير مُنجَز.

- [x] 🛡️ **RAG محلية Zero-Trust:** Qdrant + Ollama (`qwen2.5:7b`) + `bge-m3` + وكلاء CrewAI + بوابة Ragas ≥ 0.95
- [x] 🔒 **بوابة PII إغلاقية الفشل بمُعرِّف الرقم القومي المصري:** (مختبر 14/14 حالة، شاملة الأرقام العربية-الهندية)
- [x] 📱 **طبقة بيانات أندرويد مشفّرة + مزامنة مؤجلة:** (Room + SQLCipher + SyncManager مع Dead Letter Queue)
- [x] 🔍 **الفحص الجنائي الاسترشادي للمحررات (OpenCV SSIM):** مكوّنات الفروق الموضعية + فرق كثافة الحبر كاشفاً الإضافات اللاحقة — المحور الأول في خدمة ضحايا إيصالات الأمانة (نتائج استرشادية فقط؛ الإثبات بخبرة الطب الشرعي)
- [x] ⚖️ **فهرسة مبادئ النقض الثلاثة** (الجهالة / انتفاء ركن التسليم / البلاغ الكاذب والابتزاز) في مجموعة `egypt_cassation_rulings` مخصصة — المبادئ مجمعة ومستقرة، وعلم `needs_citation_review` يظهر مع كل إجابة حتى اعتماد المستشار القانوني لأرقام الطعون
- [x] 🔔 **تنبيهات تطورات القضايا عبر n8n لقناة مغلقة** مع إخلاء المسؤولية الإلزامي (`n8n/workflows/case-alerts.json`)
- [x] ⏱️ **حاسبة المواعيد الجنائية بالتحقق المزدوج** (محركان مستقلان + تخطي الجمعة/السبت): المعارضة 10 أيام، الاستئناف 10، النقض 60 — مُتحقَّق منها على 25,933 يوماً (1990–2060)؛ أرقام المواد الثلاثة بانتظار الاعتماد القانوني
- [x] 🗂️ **واجهة الوصول الشامل:** وضع الاستغاثة بثلاثة أزرار عملاقة + 4 ثيمات عالية التباين (AAA) + قراءة صوتية متزامنة + تكبير خط 200%
- [ ] 🔤 **OCR عربي على الجهاز (ML Kit):** الاعتمادية في الكتالوج وقواعد ProGuard جاهزة — لم تُربَط بعد بالبناء
- [ ] 🏷️ **تدريب نموذج YOLO خفيف على الأختام والتوقيعات المصرية** (ختم النسر) — يُفعَّل تلقائياً عند تركيبه
